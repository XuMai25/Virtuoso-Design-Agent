...
- `vda onboarding-resolve` 把 hash-bound 草案与显式 `user_input` resolution 编译成现有 planner 可接受的普通 `existing_schematic simulation.run/design.tune` TaskSpec。target、PDK、topology hash、冻结对象和 child scope 只能从草案继承；role 必须精确引用 OA graph，每个授权 CDF 字段必须存在于 inventory 且拥有一对一 OA→`si` binding，source/load/transfer 不能引用未知 top net，hierarchy terminal order 必须完整覆盖已 inspect terminal set。编译输出的远端计算/写入开关固定为 false。保留的真实 flat 草案已编译出 AC 计划 token `2edc16bd357b2dc6`；hierarchy 草案已编译出两点 shared-netlist tuning 计划 token `74dcb84202d70fb9`。这两个历史结果仍只是本地契约 Gate。
