...
不需要人工窗口的已保存 setup 可用 `ade.run`。它要求 `target.view: "maestro"`、`ade_run` 设置和 `allow_remote_compute: true`，但不要求 `allow_remote_write`；worker 新开 background session，回读 tests，运行 setup，并用本次调用返回的 history 读取逐点 output/spec。默认 `require_structured_outputs: true`；显式设为 false 且没有输出时，run record 仍降为 `partial`。该 operation 不配置/保存 setup，不捕获 Spectre input/PSF 哈希，也不把 ADE output 自动映射成 VDA constraints。history 命名/覆盖策略由已保存 setup 决定，VDA 当前不能在后台 run 前证明名称唯一，计划会明确披露。background 产物指纹、有限 corner 和 live smoke 仍是后续 Gate；人工相关验证见 `docs/deferred-manual-gates.md`。
