...
当前版本从 **L5A** 起步：在已知 PDK、固定电路模板、显式规格和有限搜索空间内完成闭环。TSMC28 反相器 Gate 1，以及电阻负载 NMOS 共源/源极退化的 DC、复数 AC、W/L/RD/RS、bias/load、相干 transient 线性度、真实 VDD 功耗、ordinary noise 和固定三分析 `quality` 均有真实 OA/`si`/Spectre 证据。2026-07-22 又在既有 `vda_cs_ac_tradeoff_001` 上完成 `L=[0.03,0.04] µm × VDD=[0.8,0.9] V` 四点质量搜索、最佳 OA 写回，并对同一 OA/同一 `si` 网表执行 TT/25℃/0.90V、SS/125℃/0.81V、FF/−40℃/0.99V 三条件共九项 Spectre 分析。2026-07-23 再把同一有限 PVT 集合可选接入 `design.tune`，并在新共源 cell 上闭合源极退化 add/remove 与原始实例 CDF `fingers` 两点自动搜索、同源网表证明、最佳 OA 写回和 transport checkpoint 恢复。PVT 不会默认附加到普通任务；真实 OA 设计变量的跨 PVT 写回仍是可选后续 Gate。同日开始 Gate 3：固定电阻负载 NMOS 差分对的建图、精确回读、对称 W/L/R 写入、OA→`si`→Spectre nominal DC、双支路 KCL/工作区/偏移/功耗判定和有限搜索已完成本地实现与离线闭环，但还没有 live TSMC N28 证据。差分 AC/CMRR、输入共模范围、PVT、ADE multi-test/multi-analysis、人工打开/修改/重跑和旧 ADE L 非覆盖迁移仍未闭合，因此不能称为完整 L5B 设计质量闭环。
...
- Gate 3 差分对复用同一 worker 与 executor，不复制 Bridge：OA DUT 只有 `MN0/MN1/RD0/RD1` 和 `INP/INN/OUTP/OUTN/TAIL/VDD/VSS`；理想尾电流源与共模输入只在 wrapper 中。当前 nominal DC 路径核对双管/双负载对称、OA 与 `si` 的单指宽/指数量/总宽/L/R、一对器件节点与 OP 值、尾电流/电源/两只负载三类 KCL，并输出支路失配、双管饱和余量、输出偏移/共模、摆幅、gm/gds/intrinsic gain 和真实 VDD 功耗。该项目前只有本地测试与 `software_inference` demo，不得表述为 live EDA 通过。
