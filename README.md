...
- 将任务编译为带副作用标记的稳定执行计划。
- 单独规划或执行：`device.characterize`、`schematic.create`、`schematic.inspect`、`schematic.symbol.generate`、`schematic.transform`、`parameters.apply`、`ade.prepare`、`ade.capture`、`ade.corners.apply`、`ade.variables.apply`、`ade.setup.apply`、`ade.run`、`simulation.run`、`design.tune`、`design.close_loop`。`device.characterize` 只有远端 scratch/compute，不接受 OA target；`schematic.symbol.generate` 只为精确绑定且尚无 symbol 的 existing schematic 非覆盖生成 sibling symbol，并独立回读；当前 `schematic.transform` 开放共源级源极退化的受控 add/remove、反相器 core→ADE source/load testbench、差分对 core→`MNTAIL/BIAS`、真实尾管差分对的对称源极退化 add/remove，以及无源退化真实尾管差分对的 `RD0/RD1 ↔ MP0/MP1` 电流镜负载可逆变换。预声明通用 delta 还可把既有共源级原位增量变成共栅级，不需要重建整个 cellview；该变体的 OA pin/placement、`si`、DC/AC 和 inverse 已有 live 证据。
...
- `vda onboarding-resolve` 把 hash-bound 草案与显式 `user_input` resolution 编译成现有 planner 可接受的普通 `existing_schematic simulation.run/design.tune` TaskSpec。target、PDK、topology hash、冻结对象和 child scope 只能从草案继承；role 必须精确引用 OA graph，每个授权 CDF 字段必须存在于 inventory 且拥有一对一 OA→`si` binding，source/load/transfer 不能引用未知 top net，hierarchy terminal order 必须完整覆盖已 inspect terminal set。输出远端计算/写入开关固定为 false。保留的真实 flat 草案已编译出 AC 计划 token `2edc16bd357b2dc6`；hierarchy 草案已编译出两点 shared-netlist tuning 计划 token `74dcb84202d70fb9`。本轮仍是本地契约 Gate，不是新的 OA/Spectre 结果。
