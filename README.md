...
当前受测 Bridge 执行底座为上游 `0.8.0`/main `c64461c` 加 VDA 私有安全与稳定性 merge `106c61e`，以及 Windows managed-daemon restart 修正 `40ff891`（文档 tip `0156579`）。升级保留 RAMIC/SSH 回环、Windows 隐藏 tunnel、完整 JSON framing、有限重试、pre-send 恢复、用户/绑定核验与退出清理，同时接入上游 split-host、Paramiko/SOCKS5、scoped Spectre pools、严格 PSF、schematic netlist/planner 和 Maestro 修复。逐文件差异、备份、测试与未验证边界见[第三方补丁记录](docs/third-party/virtuoso-bridge-local-patch.md)和[升级验证](docs/validation/2026-09-04-bridge-upstream-0.8-upgrade.md)。
...
- 上述能力已经收敛为 [默认快速 preview shortlist 工作流](docs/fast-preview-shortlist-workflow.md)：4 个及以上候选先估算是否值得 preview，默认一次批量 preview、冻结 top-3、一次 checkpointed OA shortlist，昂贵质量分析只对真实 winner 运行。Bridge 0.8 下同一批次复用一次 guard、一个 simulator 和一次远端 inventory，以最多四路 scoped pool 执行；结果按提交顺序与远端 deck 路径双重绑定，旧证据格式仍可读。相同十变体 live 任务的 simulation action 由 `70.566828 s` 降至 `40.450857 s`，893 个指标和十个 deck hash 全部一致。完整 OA 域只做代码/PDK 变化、失效/近边界或周期性审计，不再为了证明流程而立即换拓扑重跑。
...
- `existing_schematic` 提供不依赖固定电路模板的 Bridge 能力面：`schematic.inspect` 保留 Bridge 的完整结构结果和所有可回读 CDF 参数；`parameters.apply` 可按实例透传 Bridge 接受的参数字符串，写入后用定向 CDF 读取再次核对。若实际 CDF 字段已知但 `si` 参数名未知，`parameters.binding.discover` 以 exact topology、完整目标实例 CDF 表和字段权限为 CAS，执行 baseline→单字段 probe→restore 三次 netlisting。最简单的一对一结果输出 `direct_literal_binding`；foundry PDK callback 引起多个字段变化时，只有同一目标实例中存在唯一 literal 主字段、且每个额外 netlist 字段都由同名 OA callback 的 before/after 数值逐项解释，才输出 `direct_literal_binding_with_derived_callbacks`，并完整保留副作用。跨实例、结构变化、多个主候选、未镜像多字段或零变化仍拒绝提升。2026-08-01 已用 TSMC N28 `MNCAS.Wfg 750.0n→800n→750.0n` 真实验证 `Wfg→w` 与 `ad/as/nrd/nrs/pd/ps` 派生 callback、完整 CDF/netlist 恢复和精确 scratch 清理；未运行 Spectre。`onboarding-resolve-bindings` 现在能重新校验 discovery task/run、原始 netlist/log/manifest、完整 CDF 与恢复签名，把主 binding、派生 callback 和 source hashes 直接编译进普通安全 `simulation.run`/`design.tune` TaskSpec；后续每次 OA→`si` 都逐项复核 callback，而不是只保存说明文字。首个 normal shared-netlist DC/AC run 已真实通过：`Wfg/w` 与六个 callback 全匹配、一次 OA 回读/一次 `si`、无 OA 写入；`execution-scope` 和 `onboarding-binding-audit` 分别固化最小授权范围与事后 hash/manifest/规格复判。详见 [本地编译 Gate](docs/validation/2026-08-01-onboarding-discovered-binding-promotion-local.md)和[live Gate](docs/validation/2026-08-01-onboarding-discovered-binding-promotion-live.md)。反相器/共源模板仍可在同一任务中组合 semantic parameters 与原始实例参数。固定模板与通用既有 schematic 都可用 `instance_parameter_space` 或原子 `candidate_set` 声明有限的 `instance.parameter -> raw strings` 域；搜索字段名必须来自未过滤 OA inspect 的实际 CDF 名，不猜 Bridge 别名。这不收窄独立 `parameters.apply` 的原有 Bridge 能力。
...
如果真实 probe 已完成，但分类规则随后升级，不需要再次写 OA 或重复三次 `si`。可对保存的
real-Bridge run 做纯本地 artifact/恢复证据复判：

```powershell
.\.venv\Scripts\vda.exe binding-reclassify `
  artifacts\runs\binding-discovery\probe-run.json `
  --output artifacts\runs\binding-discovery\reclassification.json
```

该命令要求 source action 是成功的 `eda_result`，重新哈希三阶段 netlist/log/manifest，并核对完整
OA 表与 canonical signature 恢复。输出只属于 `software_inference`，明确不连接远端、不写 OA，
也不改写原 run record。

将一个或多个已执行 discovery task/run 自动接入 onboarding intent，不手抄网表字段：

```powershell
.\.venv\Scripts\vda.exe onboarding-resolve-bindings `
  artifacts\onboarding\draft.json `
  examples\onboarding\resolution-without-bindings.json `
  --binding-source artifacts\binding-task.json artifacts\binding-run.json `
  --output artifacts\resolved.safe.json `
  --record-output artifacts\binding-promotion.json
```

resolution 必须先明确授权目标 OA 字段，但 `generic_simulation.netlist_parameter_bindings` 要留空给
编译器填入；已有人工 mapping 冲突、target/PDK/topology/CDF hash 漂移、未恢复或不可提升的
discovery 一律拒绝。输出 binding 内含 discovery task/run SHA-256，并将派生 callback 作为运行期
一致性检查；不会自动打开 `allow_remote_compute` 或 `allow_remote_write`。

从安全任务确定性生成等待确认的最小执行范围，并在真实 shared-netlist run 后复判完整 handoff：

```powershell
.\.venv\Scripts\vda.exe execution-scope `
  artifacts\resolved.safe.json `
  --output artifacts\resolved.live.json `
  --record-output artifacts\execution-scope.json

.\.venv\Scripts\vda.exe onboarding-binding-audit `
  artifacts\binding-promotion.json `
  artifacts\execution-scope.json `
  artifacts\resolved.live.json `
  artifacts\run.json `
  --output artifacts\binding-run-audit.json
```

`execution-scope` 只开启 plan 已有步骤实际要求的 compute/write 权限，拒绝覆盖任务，输出新 token 但
不执行且不代替用户确认。`onboarding-binding-audit` 目前只接受成功的 compute-only shared-netlist
`simulation.run`，会重新核对 promotion/scope/task/run SHA、token、OA topology、主字段、全部
callback、两阶段 manifest 和规格。真实 Gate 见
[discovery→normal-task live 记录](docs/validation/2026-08-01-onboarding-discovered-binding-promotion-live.md)。
...
或 Maestro，也不写 OA；一个批次使用唯一 `/data/xum/.../vda_netlist_preview_*` 根，每个变体由 Bridge 分配唯一子目录
...
- [2026-08-01 existing-schematic CDF→si binding 自动发现真实 Gate](docs/validation/2026-08-01-existing-schematic-parameter-binding-discovery-live.md)
- [2026-08-01 discovery→normal-task 编译本地 Gate](docs/validation/2026-08-01-onboarding-discovered-binding-promotion-local.md)
- [2026-08-01 discovery→normal-task OA/si/DC/AC 真实 Gate](docs/validation/2026-08-01-onboarding-discovered-binding-promotion-live.md)
