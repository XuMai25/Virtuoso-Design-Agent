...
当前版本从 **L5A** 起步：在已知 PDK、固定电路模板、显式规格和有限搜索空间内完成闭环。TSMC28 反相器 Gate 1，以及电阻负载 NMOS 共源/源极退化的 DC、复数 AC、W/L/RD/RS、bias/load、相干 transient 线性度、真实 VDD 功耗、ordinary noise 和固定三分析 `quality` 均有真实 OA/`si`/Spectre 证据。2026-07-22 又完成共源 L/VDD 质量搜索、最佳 OA 写回和固定设计 TT/SS/FF 三条件验证；PVT 作为显式可选项接入 testbench 调优，但不会默认附加。2026-07-23 先在 `vda_diffpair_gate3_001` 闭合理想尾源差分对 Gate 3，随后在全新 `vda_diffpair_tail_gate4_001` 上以严格增量 transform 加入 OA `MNTAIL/BIAS`，完成真实尾管 bias 与 W checkpoint、OA→`si` 参数一致性、DC/KCL/工作区、成对差模/共模 AC、带宽/GBW/unity/CMRR、10 点 ICMR、平衡 transient THD/P1dB 和 differential noise。可选差分对 PVT、mismatch/Monte Carlo、ADE multi-test/multi-analysis、人工打开/修改/重跑和旧 ADE L 非覆盖迁移仍未闭合，因此不能称为完整 L5B 设计质量闭环。
...
- 将任务编译为带副作用标记的稳定执行计划。
- 单独规划或执行：`schematic.create`、`schematic.inspect`、`schematic.transform`、`parameters.apply`、`ade.prepare`、`ade.capture`、`ade.corners.apply`、`ade.variables.apply`、`ade.setup.apply`、`ade.run`、`simulation.run`、`design.tune`、`design.close_loop`。当前 `schematic.transform` 开放共源级源极退化的受控 add/remove、反相器 core→ADE source/load testbench，以及差分对 core→`MNTAIL/BIAS` 的固定小变更。
...
- Gate 3/4 差分对复用同一 worker 与 executor，不复制 Bridge。Gate 3 保留外部理想尾源能力；Gate 4 的严格 transform 只新增 `MNTAIL(TAIL,BIAS,VSS,VSS)` 与 `BIAS` pin，保持 `MN0/MN1/RD0/RD1`、已有 nets/pins 和 placement 不变。`tail_width_um/tail_length_um` 属于 OA semantic 参数，`tail_bias_v` 只属于 wrapper；真实尾管路径拒绝理想 `tail_current_ua/tail_output_resistance_ohm`。同一 OA→`si` 证据链现已 live 覆盖 MNTAIL `ids/vgs/vds/vdsat/gm/gds`、三类 KCL、bias/width 调优、差模/共模 AC、CMRR/ICMR、相干 transient 与单一差分输入源 noise。PVT、mismatch 和 ADE handoff 仍是边界。
