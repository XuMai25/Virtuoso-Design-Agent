...
当前受测 Bridge 执行底座为上游 `0.8.0`/main `c64461c` 加 VDA 私有安全与稳定性 merge `106c61e`（文档 tip `731b67f`）。升级保留 RAMIC/SSH 回环、Windows 隐藏 tunnel、完整 JSON framing、有限重试、pre-send 恢复、用户/绑定核验与退出清理，同时接入上游 split-host、Paramiko/SOCKS5、scoped Spectre pools、严格 PSF、schematic netlist/planner 和 Maestro 修复。逐文件差异、备份、测试与未验证边界见[第三方补丁记录](docs/third-party/virtuoso-bridge-local-patch.md)和[升级验证](docs/validation/2026-09-04-bridge-upstream-0.8-upgrade.md)。
