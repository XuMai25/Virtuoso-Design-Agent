...
当前版本从 **L5A** 起步：在已知 PDK、固定电路模板、显式规格和有限搜索空间内完成闭环。TSMC28 反相器 Gate 1，以及电阻负载 NMOS 共源/源极退化的 DC、复数 AC、W/L/RD/RS、bias/load、相干 transient 线性度、真实 VDD 功耗、ordinary noise 和固定三分析 `quality` 均有真实 OA/`si`/Spectre 证据。2026-07-22 又完成共源 L/VDD 质量搜索、最佳 OA 写回和固定设计 TT/SS/FF 三条件验证；PVT 作为显式可选项接入 testbench 调优，但不会默认附加。2026-07-23 闭合共源可逆 source-degeneration 与原始 CDF `fingers` 搜索后，又在新 `vda_diffpair_gate3_001` 上完成差分对 non-overwrite 建图、nominal DC、18 点设计参数搜索与写回、不可行/预算/transport 恢复、差模 AC/GBW、有限尾源输出电阻下的 CMRR/CMRR 带宽、输入共模采样、平衡差分 transient THD/P1dB，以及尾电流×负载只读 AC 选优。差分对真实尾管/偏置、noise、可选 PVT、ADE multi-test/multi-analysis、人工打开/修改/重跑和旧 ADE L 非覆盖迁移仍未闭合，因此不能称为完整 L5B 设计质量闭环。
...
- Gate 3 差分对复用同一 worker 与 executor，不复制 Bridge：OA DUT 只有 `MN0/MN1/RD0/RD1` 和 `INP/INN/OUTP/OUTN/TAIL/VDD/VSS`；尾源、共模输入、有限尾源输出电阻和对称输出负载只在 wrapper 中。nominal DC 核对双管/双负载对称、OA/`si` geometry、节点/OP 和三类 KCL；差模 AC 提取 gain/带宽/GBW/unity，配对共模 AC 提取低频 CMRR 与 CMRR 带宽，平衡 transient 提取差分增益、HD2/HD3、THD、P1dB 和真实 VDD 功耗。新 TSMC N28 cell 的写回/恢复和这些动态路径均已有 live 证据；真实尾管、noise、PVT 与 ADE handoff 仍是边界。
