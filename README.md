...
当前版本从 **L5A** 起步：在已知 PDK、固定电路模板、显式规格和有限搜索空间内完成闭环。TSMC28 反相器 Gate 1、电阻负载 NMOS 共源级 nominal DC，以及同一已有 cellview 上的源极退化 transform/DC/RS 有限调优均有真实 OA/`si`/Spectre 证据。2026-07-20 又真实通过共源 nominal/退化复数 AC、12 点不写 OA 的 bias/load 条件搜索、专用 cell 上的 W/RD/RS 调优，以及同一 cell 的 5 点相干 transient 线性度/真实 VDD 功耗和 211 点普通 noise PSF。2026-07-21 固定 `quality` 组合先通过真实只读 bias/load 搜索，随后又完成 8 点 W/RD/RS 三分析搜索、逐候选 OA 写入、transport checkpoint 恢复、最佳写回和全不可行恢复。一个线性度优先任务进一步自动把 RS 从 1 kΩ 改为 2 kΩ，以 29.30% GBW 损失换取 41.93% THD 降低、37.26% P1dB 提升和 17.27% DC 功耗降低。ADE 方向现有六个正交 operation：非覆盖新建 Maestro test 的 `ade.prepare`、捕获人工聚焦 setup/history/产物的 `ade.capture`、后台运行或恢复已保存原生 analysis/sweep 的 `ade.run`、add-only 新增 enabled corner 的 `ade.corners.apply`、按 tests/corners/逐 scope 旧值 CAS 修改 design variable 或 global selection 的 `ade.variables.apply`，以及对 analysis 做旧状态 CAS 并新增命名 output/spec 的 `ade.setup.apply`。其中 `prepare → setup.apply → run` 已在 nics4304/TSMC N28 上真实闭合：新 Maestro view 指向既有反相器 schematic，transient/output/spec 持久化回读，后台结果 `VoutAvg=364 mV`，并把 exact `input.scs` 与 test design、OA 节点和 19 组 raw 参数映射核对一致。2026-07-22 又在专用反相器 testbench 上真实保存 global `CL=1f,2f,4f`，运行 Maestro 原生 3 点 sweep，并把 OA `CL0.c=CL`、runtime `input.scs`+`netlist` 输入束、exact-history RDB/log 和三个 point 的 `VoutAvg` 闭合。`capture`、人工打开/修改/重跑、旧 ADE L 迁移和数值交叉检查仍待 live Gate。共源 L/VDD 联合搜索、真实 process/temperature corner 和 multi-test/multi-analysis ADE 映射也未闭合，因此仍不能称为完整 L5B 设计质量闭环。

同一反相器 Maestro view 随后完成第一个 ADE 规格 Gate：add-only 保存并固定 delay、rise/fall skew 和每周期总供电能量的 calculator expressions，从 `Interactive.1` 的严格 RDB sweep 读取三点 scalar，以显式 SI-to-ps/fJ scale 进入 VDA constraints/objective。之后又把 `VDD0.vdc` 与 `VIN0.v2` 绑定到同一个 OA/Spectre `VDD` 符号，add-only 新增 VDD-aware outputs，并真实闭合 `VDD=[0.8,0.9] V × CL=[1,2,4] fF` 六点。五点满足 delay≤6 ps、skew≤5 ps、周期供电能量≤5 fJ；最小能量目标选择 `0.8 V/1 fF`，得到 delay 3.254 ps、skew 1.425 ps、周期供电能量 1.309 fJ。固定 `Interactive.2` 恢复没有重算或写 OA/setup。旧固定阈值 `Rise/RiseFallSkew` 在三个 0.8 V 点产生的 6 个 `eval err` 被逐点显式声明、与 RDB/log 精确对齐且未参与 metric mapping，未解释错误为 0；默认任务仍要求零错误。随后 add-only 加入 `VDA_LOW_VDD/VDA_NOMINAL_VDD`，把 `CL=1f,4f` 移到 test scope、把 VDD 分别固定为 global 0.9 V 与 named-corner 0.8/0.9 V，并显式禁用 global CL selection。真实 `Interactive.4/5` 均完成 2 个 Maestro parametric point × 3 个 corner 列；VDA 从原始 Detail CSV 保留六个二维单元并再次选中 `0.8 V/1 fF`。这只验证同一 `top_tt` 模型下的 VDD 环境 corner，不是 process corner。下一自动化硬门转为共源 L/VDD + 多 analysis + 有限真实 PVT corner。
