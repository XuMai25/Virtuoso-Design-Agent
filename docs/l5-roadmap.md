...
同日进一步加入 topology-independent small-signal matrix core，避免把上述局部校准误当作通用理论能力。新的器件表按 model/polarity/L/偏置保存 width-normalized `Id/W、gm/Id、gds/Id、gmb/Id` 和端子电容，网络请求只描述 MOS/R/C 节点图、固定 AC 边界和输入/输出表达式，不接受 topology 分类。共源、源退化共源、NMOS 差分对和 PMOS 共源已用同一矩阵组装路径通过解析测试；浮空、偏置漂移和真实数据缺 hash 会拒绝。状态为 **topology-independent small-signal matrix core locally verified; live PDK characterization and OA/si graph binding pending**。这不是任意拓扑综合：非线性 DC、`si` 图转换、真实 PDK 表和 held-out topology Spectre 误差仍待 Gate。
...
反相器 L5A
  ...
  -> theory-first gm/Id 尺寸估算（本地方程/离散域穷尽/最优性边界已实现；Gate 6 topology-local Spectre 校准已过，独立 TSMC N28 MOS 表仍待 Gate）
  -> 通用 MOS 小信号网络（器件表契约、MOS/R/C 矩阵、可脚本化 stamp/MNA 数值接缝与跨拓扑本地测试已实现；正式 JSON 新器件、真实 PDK 表、si 图绑定和留出拓扑验证待 Gate）
...
反相器可靠性 Gate 1R、共源 nominal DC、显式实例字段、源极退化可逆 transform/DC/AC、quality、W/RD/RS/L/VDD 写回、固定设计 TT/SS/FF、可选 PVT-aware bias 选优，以及差分对 nominal、真实尾管、对称源极退化和 PMOS 电流镜有源负载四条路径均已有 live 证据。Gate 5 已证明源极退化支持严格增量 add、全分析迁移、有限搜索、最佳写回、checkpoint 与精确 remove/restore；Gate 6 又证明 active-load delta 能无缝进入同一 DC/AC/CMRR/ICMR/transient/noise、搜索、失败、预算和恢复状态机。理论先导层现已能从完整声明的 gm/Id 离散表域推导 W/电流并限制最优性措辞；当前 Gate 6 的一阶增益/带宽模型又已用六点真实 TSMC N28 同源数据完成局部拟合、留一验证和新鲜复跑。其下方新增的通用 MOS/R/C 矩阵核心已通过多个 synthetic 拓扑；公开的低层系数/RHS stamping 与 raw complex solver 又允许 Agent 为具体电路增加局部 stamp 或 MNA 辅助未知量，不必复制求解器。该脚本接缝不等于新器件已经成为正式 VDA operation；尚未取得独立 MOS characterization 表或 `si` 图绑定，所以仍不能用 synthetic 推荐值指导 OA 写入。下一理论 Gate 应先用只读 PDK characterization 建表并绑定 raw `eda_result` artifact/hash，再把 `si` 图映射到通用网络，并用未参与建表/调参的完整拓扑复核；通过后，理论推荐才可缩小真实搜索域。PSRR 三次同网表 AC 已完成 nominal、四点 bias/load 只读和三种 L 的八点 OA 搜索；后者把带内最差值从 `11.5125 dB` 提升到 `19.7438 dB`，并在零可行点时正确恢复基线，但仍差临时门 `0.2562 dB`。任何近门或过门点都要补做 CMRR、线性度、噪声和必要的可选 PVT，不能仅按“最大 PSRR”宣称闭合。跨 PVT 不默认附加；slew/settling、输出驱动/摆幅边界、差分对 PVT、mismatch/Monte Carlo、ADE 真实 PVT/multi-test、差分对 setup、已有 output 安全替换，以及人工打开/修改/重跑和旧 ADE L 迁移继续是独立 Gate。若继续拓扑能力，则为 `RS0/RS1 + MP0/MP1` 单独定义组合 Gate；这些完成前仍不能升级为可重复的 L5B 单模块规格闭环。
