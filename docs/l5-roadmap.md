...
反相器 L5A
  ...
  -> 通用 MOS 小信号网络（器件表契约、MOS/R/C 矩阵与跨拓扑本地测试已实现；真实 PDK 表、si 图绑定和留出拓扑验证待 Gate）
