...
- ADE 自动链已在专用 Maestro cell 真实通过 `prepare → transient/output/spec CAS patch → background run → Interactive.0 result/manifest → OA/input.scs raw parameter consistency`，并验证同一 history 在证据失败后可不重算恢复。2026-07-22 又在 `vda_ade_sweep_inv_001` 完成受限 testbench transform、global CL 三点、VDD×CL 六点与 delay/skew/energy 规格映射。随后 add-only 新增 `VDA_LOW_VDD/VDA_NOMINAL_VDD`，把 `CL=1f,4f` 放到 test scope、VDD=0.8/0.9 放到 named-corner scope，并以 global selection CAS 禁用同名 global CL。真实 IC6.1.8 使用共享 `input.scs`+sibling `netlist` 与 exact-history RDB；Bridge 0.7.0 的普通 parser 又只保留 Nominal 列，因此 VDA 从 Bridge 公开 raw Detail CSV 保留 2 个真实 parametric point × 3 个 corner 的六格证据。`Interactive.4/5` 均有 5 个可行 case，仍选择 `0.8 V/1 fF`（3.254 ps、1.425 ps、1.309 fJ）；4 个低压 legacy error 全部解释。第二次 run 还真实通过 callback timeout 后唯一新增 completed-history 自动恢复。该 Gate 只改变同一 `top_tt` 下的 VDD，不是 process corner。下一步是共源 L/VDD + 多 analysis + 有限真实 PVT corner。需要人工打开/调整/重跑和 ADE L 迁移的 Gate 已延期记录，不再阻塞代码实现；不能用 nominal W/RD/RS 网格代替跨条件放大器设计质量。

跨拓扑的基础有两条。`existing_schematic` 可以不依赖固定模板读取已有 schematic，并用 `instance_parameter_updates` 人工指定实例原始 CDF 参数和值字符串；固定模板还可把它与 W/L/R semantic parameters 组合。写入必须经过 callback、立即定向 OA 回读和独立再次回读。ADE 路径中，`ade.prepare` 可让新 Maestro test 指向另一个既有 schematic，`ade.capture` 接收人工保存并运行的 setup/history，`ade.corners.apply` add-only 增加 corner membership，`ade.variables.apply` 对 global/test/corner 值和 global selection 执行旧状态 CAS，`ade.setup.apply` 对声明 analysis 做 CAS 并新增命名 output/spec，`ade.run` 后台执行或恢复已保存 setup，读取逐 point/corner output/spec，并绑定 exact-history result/log 与 runtime input manifest；可选 `sweep_verification` 进一步要求 setup/selection/OA/input/result 一致，可选 `result_mapping` 再固定保存的 output expression、单位换算、constraints 和 objective。除人工 `capture` 外，上述自动路径的主要切片均有 nics4304 live 证据；仍不证明真实 process/temperature corner、history 名唯一或 multi-test 通用映射。`ade.run` 已证明 global 与 test-scope CL/VDD 环境 corner 通过共享符号输入束进入 RDB/Detail；它不把 `Wfg` 的 PDK CDF 派生语义伪装成字面 `w`。通用 OA smoke 已枚举 MN0 的 233 个 CDF 字段；专用新 cell 上又真实闭合 `MN0.fingers=2` 和 `RD0.r=22K` 的双重回读。`MN0.m=2` 被当前 PDK callback 恢复为 `1`，因此保留为字段不可持久化边界。这些能力只证明“按名字修改并以 OA 值确认”或“准备、修改声明 setup 范围、运行当前 ADE 状态”，不证明 VDA 理解任意参数的物理作用，也不自动允许该参数参与调优。
