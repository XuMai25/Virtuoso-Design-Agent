...
- ADE 自动链已在专用 Maestro cell 真实通过 `prepare → transient/output/spec CAS patch → background run → Interactive.0 result/manifest → OA/input.scs raw parameter consistency`，并验证同一 history 在证据失败后可不重算恢复。2026-07-22 又在 `vda_ade_sweep_inv_001` 完成受限 testbench transform、`CL0.c=CL` 双重 OA 回读、global `CL=1f,2f,4f` 保存和原生 3 点 transient。真实 IC6.1.8 没有保留逐点输入目录，而是共享 `input.scs`+sibling `netlist` 与 exact-history RDB；VDA 要求 include 关系、两份输入 hash、OA/Spectre 符号绑定、setup 前后指纹、RDB 全点参数/非空 output 和 completion log 共同通过。随后 add-only 保存并固定 delay/skew/周期供电能量表达式，把 RDB scalar 以显式 SI-to-ps/fJ scale 映射到 constraints/objective。再把 `VDD0.vdc` 与 `VIN0.v2` 绑定到同一个 `VDD` 符号，新增 global `VDD=0.8,0.9` 和七个 VDD-aware outputs，真实完成 `VDD×CL` 六点；五点通过约束，按最小能量选择 `0.8 V/1 fF`，delay=3.254 ps、skew=1.425 ps、能量=1.309 fJ。六个旧固定阈值 output error 被 exact point 契约完整解释，未解释错误为零；固定 history 恢复没有重算或写 OA/setup。下一步是有限 corner/test-scope，再进入 L/VDD+corner。需要人工打开/调整/重跑和 ADE L 迁移的 Gate 已延期记录，不再阻塞代码实现。通过这些项后再进入差分对；不能用 nominal W/RD/RS 网格代替跨条件放大器设计质量。

跨拓扑的基础有两条。`existing_schematic` 可以不依赖固定模板读取已有 schematic，并用 `instance_parameter_updates` 人工指定实例原始 CDF 参数和值字符串；固定模板还可把它与 W/L/R semantic parameters 组合。写入必须经过 callback、立即定向 OA 回读和独立再次回读。ADE 路径中，`ade.prepare` 可让新 Maestro test 指向另一个既有 schematic，`ade.capture` 接收人工保存并运行的 setup/history，`ade.variables.apply` 对 global/test/corner 声明 scope 执行旧值 CAS，`ade.setup.apply` 对声明 analysis 做 CAS 并新增命名 output/spec，`ade.run` 后台执行或恢复已保存 setup，读取逐点 output/spec，并绑定 exact-history result/log 与 runtime input manifest；可选 `sweep_verification` 进一步要求逐 point 的 setup/OA/input/result 一致，可选 `result_mapping` 再固定保存的 output expression、单位换算、constraints 和 objective。`prepare/setup.apply/run`、global variable apply、单变量与二维 sweep verification、单 test scalar constraint mapping 已有 nics4304 live 证据；`capture`、test/corner variable scope、人工 ADE 与 corner 尚无 live。`ade.run` 可证明当前单点测试的 OA raw `l/w/nf/multi`、源和负载参数进入真实 `input.scs`，也已实机证明 global `CL/VDD` sweep 通过共享符号输入束进入 RDB 逐点结果；仍不证明 history 名唯一、multi-test 通用映射或把 `Wfg` 的 PDK CDF 派生语义伪装成字面 `w`。通用 OA smoke 已枚举 MN0 的 233 个 CDF 字段；专用新 cell 上又真实闭合 `MN0.fingers=2` 和 `RD0.r=22K` 的双重回读。`MN0.m=2` 被当前 PDK callback 恢复为 `1`，因此保留为字段不可持久化边界。这些能力只证明“按名字修改并以 OA 值确认”或“准备、修改声明 setup 范围、运行当前 ADE 状态”，不证明 VDA 理解任意参数的物理作用，也不自动允许该参数参与调优。
