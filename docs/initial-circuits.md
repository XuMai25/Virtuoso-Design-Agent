...
- 当前最近的自动化硬门：global/test/corner 变量逐 scope 旧值 CAS、analysis CAS + output/spec add-only patch，以及 `ade.run` 对本次确切 history 的 core netlist/result/log 大小与 SHA-256 清单均已完成本地契约。下一步是在专用 Maestro cell 做“setup patch → background run → result/manifest” live smoke，并从 manifest 对应的真实网表核对声明变量；随后加入已有 output 安全替换、L/VDD 和有限 corner，检查同一设计在多个工作条件下能否重复满足规格。需要人工打开/调整/重跑和 ADE L 迁移的 Gate 已延期记录，不再阻塞代码实现。通过这些项后再进入差分对；不能用 nominal W/RD/RS 网格代替跨条件放大器设计质量。

跨拓扑的基础有两条。`existing_schematic` 可以不依赖固定模板读取已有 schematic，并用 `instance_parameter_updates` 人工指定实例原始 CDF 参数和值字符串；固定模板还可把它与 W/L/R semantic parameters 组合。写入必须经过 callback、立即定向 OA 回读和独立再次回读。ADE 路径中，`ade.prepare` 新建空的持久化 Spectre test，`ade.capture` 接收人工保存并运行的 setup/history，`ade.variables.apply` 对 global/test/corner 声明 scope 执行旧值 CAS，`ade.setup.apply` 对声明 analysis 做 CAS 并新增命名 output/spec，`ade.run` 后台执行已保存 setup 的原生 analysis/sweep，读取逐点 output/spec，并绑定 exact-history input/result/log manifest。参数路径已有 live 证据；五个 ADE operation 当前都只有本地实现与失败测试，尚无 nics4304 live 证据；两个 patch 都只证明声明范围，`ade.run` 的路径/哈希也不证明 history 名唯一或变量语义已进入网表。通用 OA smoke 已枚举 MN0 的 233 个 CDF 字段；专用新 cell 上又真实闭合 `MN0.fingers=2` 和 `RD0.r=22K` 的双重回读。`MN0.m=2` 被当前 PDK callback 恢复为 `1`，因此保留为字段不可持久化边界。这些能力只证明“按名字修改并以 OA 值确认”或“准备、捕获、修改声明 setup 范围、运行当前 ADE 状态”，不证明 VDA 理解任意参数的物理作用，也不自动允许该参数参与调优。
