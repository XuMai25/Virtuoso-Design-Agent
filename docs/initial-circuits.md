...
verified at nominal top_tt**。差分对的 Wn/Wp/Wtail 六点随后也完成
**held-out local-response shortlist to same-source EDA selection verified at nominal top_tt**。
...
- 差分对六点在既有 `vda_diffpair_active_gate6_001` 完成逐点 OA 写入/回读、自动 `si`、
  双支路 DC、差模 AC、共模 AC/CMRR、最佳写回和独立 OA 回读。6/6 全规格可行；模型
  与 EDA 都选最小功耗点 `Wn/Wp/Wtail=1.215/1.080/0.555 µm`，功耗=`10.2971 µW`、
  gain=`3.71568 V/V`、BW=`2.68269 GHz`、GBW=`9.96800 GHz`、CMRR=`34.8249 dB`。
  exact validator 的 90/90 比较通过，最坏新点误差为 BW 的 `2.5693%`。
- 新 result 保存每个 metric 的误差归一化 floor；旧 result 缺少该字段时必须提供已由
  result canonical SHA 绑定的原 policy。差分对旧 record 首次按 schema 默认值得到的
  mismatch 假失败被保留为诊断证据，修复没有改变运行前声明的 floor 或误差门。
...
- 差分对首点下载 `si.env` 发生 DNS/SCP，第二、三点分别在 `si -batch` 发生
  `WinError 10054`。每次都恢复并独立回读 Gate 8 anchor，再从 index 1/2/3 续跑；最终
  6/6 完整且远端 `Spectre/si/Maestro=0/0/0`，没有运行进程泄漏。

下一道增量 Gate 转为新差分对最佳点的只读质量回归：重新运行 PSRR、noise、
transient/P1dB 和 ICMR，不能把 Gate 8 旧宽度的 follow-up 结果直接外推。执行前仍要
重新列出远端计算与 scratch；本轮授权没有覆盖这些 analysis。若继续调整共源 RS，必须
先增加至少一个独立 RS 留出探针，不能用缺失方向的低总误差代替。semantic+raw CDF
混合原子 tuple、新原子来源的全不可行恢复和可选 PVT 仍是后续项。
...
- onboarding resolution 编译器现能把精确 draft hash 与最终 `user_input` 角色、参数权限、typed testbench、analysis/metric 和有限候选合成为普通 `simulation.run`/`design.tune` TaskSpec。所有对象必须来自草案 inventory，权限与 OA→`si` binding 必须一一相等；输出安全开关仍关闭。flat AC 与一层 child 两点 tuning 的真实历史草案均已生成有效计划。随后新 `vda_pmos_loaded_cs_onboarding_001` 已真实走完非覆盖 create、通用 PMOS 负载 delta、draft/resolution、同源 DC、三点 OA+testbench 联合 DC/AC、checkpoint recovery 和 winner W/L 写回；完整记录见 [`2026-07-31-pmos-loaded-common-source-onboarding-live.md`](validation/2026-07-31-pmos-loaded-common-source-onboarding-live.md)。后续本地 compiler Gate 又开放 `design.close_loop`：最多三个 alternative 从 draft snapshot 本地证明 exact forward/inverse，派生 baseline/alternative context，新增实例仅接受 fixed CDF 与新增 binding，昂贵 analysis/可选 PVT 只进入 `winner_verification`；没有新增 executor 或 EDA 运行。
- post-refinement promotion compiler 现把真实 topology winner、最终 Bridge readback 与独立写后 inspect 绑定，再按实际 variant 自动选择预声明分支；fresh inventory 中的任意显式 CDF permission/binding 可进入普通固定拓扑 `design.tune`，新增实例因此可在回读后参与搜索。六项 source hash、compiled-task hash、确定性 artifact、stage-1 域耗尽、winner 唯一性、CDF 缺失/结构漂移/权限关闭拒绝与幂等重建均已本地验证；尚无新的 live EDA。
