...
| `ade.run` | 在独立后台 session 运行或恢复已保存的 Maestro setup，读取逐点 output/spec、exact-history/result/log 与唯一 runtime input 哈希；可要求 OA→`input.scs` 一致性或严格的原生 sweep 逐点 input/result 绑定 | 远端计算，不写 OA/setup |
...
`sweep_verification` 是 `ade.run` 上的可选严格附加门，不替代或收窄 Bridge 的普通 Maestro 运行能力。任务显式列出 exact tests/corners、声明 scope 的完整 sweep 字符串、从 1 连续编号的 expected points，以及每个 test/variable 对应的 OA `instance.parameter`。worker 在运行前后回读相同 setup；运行后逐 point 检查 Detail 参数与至少一个非空 scalar output，从 exact history 绑定每个 point/test 的非空 `input.scs` 和结果，再确认 OA raw 参数引用变量、Spectre 实例或 `parameters NAME=value` 采用该 point 值。point/input/result 聚合和单位等价比较属于 `software_inference`，不改变原始 `eda_result`。缺点、空 output、缺 input/result、变量 shadow 后值不符或未知 primitive contract 都硬失败。该契约已于 2026-07-22 本地通过，IC6.1.8 的真实逐点目录和变量文件形状仍需 live Gate；若真实输入通过独立 variables file 解析，必须在看到保留产物后增加窄支持，不能猜测或把 central netlist 冒充逐点证据。
