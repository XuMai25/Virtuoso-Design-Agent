...
2026-07-22 的 TSMC N28 live Gate 已在 `vda_ade_sweep_inv_001` 上保存 global `CL=1f,2f,4f`，真实运行 3 点 transient 并从 `Interactive.0` RDB 回读 `VoutAvg=415.5/419.8/428.3 mV`；completion log 为 3 点完成、0 错误。首轮运行后因错误的逐点目录假设失败，随后固定同一 history/scratch 只读恢复，没有重算。OA `CL0.c=CL`、Spectre `CL0 ... c=CL`、setup 前后指纹、两份 runtime 输入哈希和 RDB/log 均闭合。随后在同一 view add-only 保存 `Tphl/Tplh/Delay/Rise/Fall/RiseFallSkew/SupplyEnergyCycle`，`Interactive.1` 三点全部通过 delay≤6 ps、skew≤5 ps、周期供电能量≤5 fJ，并按最小能量选中 CL=1 fF（2.882 ps、1.223 ps、1.659 fJ）。表达式状态在运行前后及固定 history 恢复中保持同一指纹，恢复没有重算或写 OA/setup。之后把 `VDD0.vdc` 与 `VIN0.v2` 共同绑定为 `VDD`，新增 global `VDD=0.8,0.9` 和七个 VDD-aware outputs，`Interactive.2` 六点中五点可行，按最小能量选中 `0.8 V/1 fF`（3.254 ps、1.425 ps、1.309 fJ）。六个旧 output `eval err` 精确落在三个 0.8 V 点且全部被显式核对，未解释错误为零；固定 history 恢复没有重算或写 OA/setup。状态升级为原生二维 testbench sweep 的同源执行、物理 scalar 映射、规格判定与证据恢复已验证；corner、test/corner scope 和 multi-test/multi-analysis mapping 仍待 Gate。
