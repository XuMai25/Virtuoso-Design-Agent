...
- VDA 对现有 Maestro setup 的 global/test/corner 逐 scope 变量 CAS，以及 analysis CAS + output/spec add-only patch，均已有本地契约但仍待 live；原生 corner 搜索尚未实现。background parametric sweep run/result 回收已有本地契约，仍待 live。
