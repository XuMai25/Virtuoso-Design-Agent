...
CIRCUIT_CATALOG: dict[CircuitKind, CircuitCapability] = {
    ...
    CircuitKind.NETLIST_PREVIEW: CircuitCapability(
        ...
        evidence_gate=(
            ...
            "zero residual Spectre/si/Maestro processes. Bridge 0.8 now runs one fixed "
            "preview batch with one guard, one simulator, one remote inventory, and "
            "at most four scoped workers; the same ten-variant live action fell from "
            "70.567s to 40.451s with identical metrics and deck hashes. A hash-bound "
            "nine-point "
    ...
    CircuitKind.EXISTING_SCHEMATIC: CircuitCapability(
        ...
        evidence_gate=(
            ...
            "a hash/plan-bound read-only onboarding compiler preserves complete CDF "
            "inventories and emits frozen, zero-permission, non-executable role/testbench "
            "drafts; retained real Bridge flat and one-level hierarchy inspections each "
            "replayed 235 fields locally + a draft-hash-bound user-intent resolver rejects "
            "unknown topology objects, CDF fields, nodes, or child terminals and compiles "
            "normal safe simulation/tuning TaskSpecs; retained flat AC and hierarchy two-point "
            "tuning drafts both pass the existing planner locally with remote flags disabled + "
            "the resolver also compiles up to three draft-bound reversible topology alternatives "
            "and winner-only quality/PVT intent into the existing close-loop controller; local "
            "forward/inverse replay, derived contexts, inherited testbench bindings, alternative "
            "candidate overrides, and disabled remote flags are verified, while first live use "
            "is intentionally reserved for a real user module + "
            "a hash-bound post-refinement promotion compiler now requires one exhausted real "
            "close-loop run plus later independent winner CDF inspection, automatically selects "
            ...
            "an exact topology and complete instance CDF CAS, promotes either one literal "
            "netlist change or one unique literal primary plus fully mirrored same-instance "
            "derived callbacks, rejects cross-instance/structural/unmirrored/ambiguous/inert "
            "results, and restores OA on injected si failure; a read-only-inspection compiler "
            "now "
            "preserves the complete CDF table and source/task/intent hashes while emitting a "
            "disabled-by-default probe task; flat TSMC N28 MNCAS.Wfg->w plus callback-derived "
            "ad/as/nrd/nrs/pd/ps, exact restoration, NFS-safe cleanup, and local evidence "
            "reclassification are live verified without modifying Bridge; a hash-bound "
            "onboarding promotion compiler now embeds that primary mapping, derived callback "
            "checks, and discovery source hashes into normal disabled simulation/tuning tasks, "
            "with a minimum-scope execution compiler that still requires a new confirmation "
            "token; one compute-only shared-netlist DC/AC run has now live matched Wfg/w and "
            "all six callbacks, and a hash-bound post-run audit rechecks its OA/si, manifests, "
            "stages, and constraints without rerunning EDA + "
            ...
            "winner W/L writeback live at nominal TSMC N28 + "
